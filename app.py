...
def main():
    ...
    if uploaded_file is not None:
        ...
        df['add_hundred'] = df.iloc[:,0] + 100
        df['substract_hundred'] = df.iloc[:,0] - 100
        df['multiply_hundred'] = df.iloc[:,0] * 100
        df['divided_hundred'] = df.iloc[:,0] / 100
...
def get_table_download_link(df, filename):
    ...
    href = f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="{filename}">Excel</a>'
    return href
